/* File='scripts/claris/bundles/01_base.js': hugo.Environment='{{ hugo.Environment }}' .Page='{{ .Page }}' .MediaType='{{ .MediaType }}' */

// NOTE: JavaScript code in this file is inlined at the bottom of <body>
// This means that
// * Execution is synchronous, i.e., the script does not have the "async" attribute

import { initializePrintView } from "scripts/vitae/base/print-view";
import { onDOMContentLoaded } from "scripts/claris/base/functions";
onDOMContentLoaded(initializePrintView);
